...
namespace Tsi.Template.Services.Common
{
    ...
    { 
        ...
        public PermissionService(IRepository<PermissionUserRoleMapping> permissionUserRoleRepo
        ...
        private async Task<(UserRole, Permission)> GetPermissionAndUserRoleCoupleAsync(string permissionSystemName, string roleSystemName)
        {
            ...
            {
                throw new CoreException("Argument null exception in GetPermissionAndUserRoleCoupleAsync"
                    , new ArgumentNullException(nameof(roleSystemName), "Cannot be empty"));
            }

            if (string.IsNullOrWhiteSpace(permissionSystemName))
            {
                throw new CoreException("Argument null exception in GetPermissionAndUserRoleCoupleAsync"
                    , new ArgumentNullException(nameof(permissionSystemName), "Cannot be empty"));
            }
            ...
            {
                throw new CoreException($"Role not found: {roleSystemName}");
            }
            ...
            {
                throw new CoreException($"Permission not found: {permissionSystemName}");
            }
        ...
        {
            var currentUser = await _userRegistrationService.GetAuthenticatedUserAsync();
